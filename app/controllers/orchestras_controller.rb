class OrchestrasController < ApplicationController
  ...
  def show
    ...
    filters = params.slice(:year, :genre, :orchestra_period, :singer)
    ...
    if filters[:orchestra_period].present?
      @recordings = @recordings.joins(:orchestra).where(orchestra: {orchestra_periods: {name: filters[:orchestra_period]}})
    end

    if filters[:singer].present?
      @recordings = @recordings.joins(recording_singers: :person).where(people: {name: filters[:singer]})
    end
