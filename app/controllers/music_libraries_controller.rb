class MusicLibrariesController < ApplicationController
  ...
  def show
    ...
    @recordings = policy_scope(Recording).random.limit(36).strict_loading.includes(:composition, :orchestra, :singers, :genre, digital_remasters: [audio_variants: [audio_file_attachment: :blob], album: [album_art_attachment: :blob]])
