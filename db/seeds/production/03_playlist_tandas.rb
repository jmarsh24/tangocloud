...
attach_playlist_image(playlist, Rails.root.join("db/seeds/common/playlists/dogac_ozen.jpeg"))

Tanda.limit(12).each_with_index do |tanda, index|
  playlist.playlist_items.create!(item: tanda, position: index)
  attach_playlist_image(playlist, Rails.root.join("db/seeds/common/playlists/dogac_ozen.jpeg"))
  attach_playlist_file(playlist, Rails.root.join("db/seeds/common/playlists/dogac_la_coqueta.m3u8"))
end
...
Tanda.offset(12).limit(17).each_with_index do |tanda, index|
  playlist.playlist_items.create!(item: tanda, position: index)
  attach_playlist_image(playlist, Rails.root.join("db/seeds/common/playlists/dogac_ozen.jpeg"))
  attach_playlist_file(playlist, Rails.root.join("db/seeds/common/playlists/dogac_petit_marathon.m3u8"))
end
